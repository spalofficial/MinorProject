...
<html>
	...
	$s3=$_POST['pass_marks'];
	$c=$_SESSION["levels"];
	$sql="insert into exam values('$s1','$s2','$s3','$c')"; 
	if ($con4->query($sql) === TRUE) {
		if($_SESSION['aset']){								
		    ...
		    $qwery="update id_generator set val=val+1 where id='e'";
			$res=$con->query($qwery);
...
?>
	...
	<div class="article2">
	<?php
			$qwery="select * from id_generator where id='e'";
			$res=$con->query($qwery);
			$r=$res->fetch_assoc();
			$id=$r['id'].$r['val'];
    ?>
	<div class="form">
		    <form action="examinsert.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>EXAM_INSERT:</legend>
				EXAM_ID<br>
				<input class="input" type="text" name="e_id" value=<?php echo $id ?> readonly="readonly" required><br>
				COURSE_ID<br>
				<input class="input" type="text" name="c_id" required><br>
				PASS_MARKS<br>
				<input class="input" type="number" name="pass_marks" required><br>
