...
<html>
	...
	$s3=$_POST['timing'];
	$c=$_SESSION["levels"];
	$sql="insert into sections values('$s1','$s2','$s3','$c')"; 
	if ($con4->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="section id ".$s1." is inserted";
					$a=$_SESSION["user_id"];
		    ...
		    $qwery="update id_generator set val=val+1 where id='l'";
			$res=$con->query($qwery);
...
?>
	...
	<div class="article2">
	<?php
			$qwery="select * from id_generator where id='l'";
			$res=$con->query($qwery);
			$r=$res->fetch_assoc();
			$id=$r['id'].$r['val'];
    ?>
	<div class="form">
		    <form action="sectionsinsert.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>SECTIONS_INSERT:</legend>
				SECTION_ID<br>
				<input class="input" type="text" name="s_id" value=<?php echo $id ?> readonly="readonly" required><br>
				CLASSROOM<br>
				<input class="input" type="text" name="classroom" required><br>
				TIMING<br>
				<input class="input" type="text" name="timing" required><br>
