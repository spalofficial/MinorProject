...
<html>
	...
	set d_name='$s2', ph_number='$s3', hod='$s4' where d_id='$s1'"; 
	if ($con4->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="department_id ".$s1." is edited";
					$a=$_SESSION["user_id"];
					...
					//
					$pp=$_SESSION["h"];
					$ppp="department id : ".$s1." | department name : ".$s2." | ph number :". $s3;
	...
	<div class="article2">
		...
		<?php
			$val=$_GET['d_id'];
			$qwery="select * from department where d_id='$val'";
			$res=$con4->query($qwery);
			$r=$res->fetch_assoc();
			//
			$_SESSION["h"]="department id : ".$r['d_id']." | department name : ".$r['d_name']." | ph number :". $r['ph_number'];
            //
        ?>
		    <form action="departmentedit.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>DEPARTMENT_EDIT:</legend>
			    DEPARTMENT_ID<br>
				<input class="input" type="text" name="d_id" value="<?php echo $r["d_id"]?>" readonly="readonly" required><br>
				DEPARTMENT_NAME<br>
				<input class="input" type="text" name="d_name" value="<?php echo $r["d_name"]?>" required><br>
				PHONE_NUMBER<br>
				<input class="input" type="number" name="ph_number" value="<?php echo $r["ph_number"]?>" required><br>
				HEAD_OF_DEPT<br>
				<input class="input" type="text" name="hod" value="<?php echo $r["hod"]?>" required><br>
