...
<html>
	...
	$s1=$_POST['c_id'];
	$s2=$_POST['c_name'];
	$s3=$_POST['qualification'];
	$sql="update course
	set c_name='$s2', qualification='$s3' where c_id='$s1'"; 
	if ($con4->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="course_id ".$s1." is edited";
					$a=$_SESSION["user_id"];
					...
					//
					$pp=$_SESSION["h"];
					$ppp="Course id : ".$s1." | Course name : ".$s2." | Qualification :".$s3;
	...
	<div class="article2">
		...
		<?php
			$val=$_GET['c_id'];
			$qwery="select * from course where c_id='$val'";
			$res=$con4->query($qwery);
			$r=$res->fetch_assoc();
			//
			$_SESSION["h"]="Course id : ".$r['c_id']." | Course name : ".$r['c_name']." | Qualification :". $r['qualification'];
            //
		?>
		    <form action="courseedit.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>COURSE_EDIT:</legend>
				COURSE_ID<br>
				<input class="input" type="text" name="c_id" value="<?php echo $r["c_id"]?>" readonly="readonly" required><br>
				COURSE_NAME<br>
				<input class="input" type="text" name="c_name" value="<?php echo $r["c_name"]?>" required><br>
				QUALIFICATION<br>
				<input class="input" type="text" name="qualification" value="<?php echo $r["qualification"]?>" required><br>
