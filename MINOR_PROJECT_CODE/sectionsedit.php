...
<html>
	...
	$s3=$_POST['timing'];
	$sql="update sections
	set classroom='$s2', timing='$s3' where s_id='$s1'"; 
	if ($con4->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="section_id ".$s1." is edited";
					$a=$_SESSION["user_id"];
					...
					//
					$pp=$_SESSION["h"];
					$ppp="section id : ".$s1." | classroom : ".$s2." | timing :". $s3;
	...
	<div class="article2">
		...
		<?php
			$val=$_GET['se_id'];
			$qwery="select * from sections where s_id='$val'";
			$res=$con4->query($qwery);
			$r=$res->fetch_assoc();
			//
			$_SESSION["h"]="section id : ".$r['s_id']." | classroom : ".$r['classroom']." | timing :". $r['timing'];
            //
        ?>
		    <form action="sectionsedit.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>SECTIONS_EDIT:</legend>
				SECTIONS_ID<br>
				<input class="input" type="text" name="s_id" value="<?php echo $r["s_id"]?>" readonly="readonly" required><br>
				CLASSROOM<br>
				<input class="input" type="text" name="classroom" value="<?php echo $r["classroom"]?>" required><br>
				TIMING<br>
				<input class="input" type="text" name="timing" value="<?php echo $r["timing"]?>" required><br>
