...
<html>
	...
	$s6=$_POST['comments'];
	$c=$_SESSION["levels"];
	$sql="insert into marks values('$s1','$s2','$s3', '$s4', '$s5', '$s6','$c')"; 
	if ($con1->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="marks id ".$s1." is inserted";
					$a=$_SESSION["user_id"];
		    ...
		    $qwery="update id_generator set val=val+1 where id='m'";
			$res=$con->query($qwery);
...
?>
	...
	<div class="article2">
	<?php
			$qwery="select * from id_generator where id='m'";
			$res=$con->query($qwery);
			$r=$res->fetch_assoc();
			$id=$r['id'].$r['val'];
    ?>
	<div class="form">
		    <form action="marksinsert.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>MARKS_INSERT:</legend>
				MARKS_ID<br>
				<input class="input" type="text" name="marks_id" value=<?php echo $id ?> readonly="readonly" required><br>
				STUDENT_ID<br>
				<input class="input" type="text" name="student_id" required><br>
				TENTH<br>
				<input class="input" type="number" name="tenth" required><br>
				TWELFTH<br>
				<input class="input" type="number" name="twelfth" required><br>
				UG<br>
				<input class="input" type="number" name="ug" required><br>
				COMMENTS<br>
				<input class="input" type="text" name="comments" required><br>
