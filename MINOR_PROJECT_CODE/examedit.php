...
<html>
	...
	$s3=$_POST['pass_marks'];
	$sql="update exam
	set c_id='$s2', pass_marks='$s3' where e_id='$s1'"; 
	if ($con4->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="exam_id ".$s1." is edited";
					$a=$_SESSION["user_id"];
					...
					//
					$pp=$_SESSION["h"];
					$ppp="exam id : ".$s1." | Course id : ".$s2." | pass marks :". $s3;
	...
	<div class="article2">
		...
		<?php
			$val=$_GET['e_id'];
			$qwery="select * from exam where e_id='$val'";
			$res=$con4->query($qwery);
			$r=$res->fetch_assoc();
			//
			$_SESSION["h"]="exam id : ".$r['e_id']." | Course id : ".$r['c_id']." | pass marks :". $r['pass_marks'];
            //
        ?>
		    <form action="examedit.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>EXAM_EDIT:</legend>
				EXAM_ID<br>
				<input class="input" type="text" name="e_id" value="<?php echo $r["e_id"]?>" readonly="readonly" required><br>
				COURSE_ID<br>
				<input class="input" type="text" name="c_id" value="<?php echo $r["c_id"]?>" required><br>
				PASS_MARKS<br>
				<input class="input" type="number" name="pass_marks" value="<?php echo $r["pass_marks"]?>" required><br>
