...
<html>
	...
	$s4=$_POST['hod'];
	$c=$_SESSION["levels"];
	$sql="insert into department values('$s1','$s2','$s3','$s4','$c')"; 
	if ($con4->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="exam id ".$s1." is inserted";
					$a=$_SESSION["user_id"];
		    ...
		    $qwery="update id_generator set val=val+1 where id='d'";
			$res=$con->query($qwery);
...
?>
	...
	<div class="article2">
	<?php
			$qwery="select * from id_generator where id='d'";
			$res=$con->query($qwery);
			$r=$res->fetch_assoc();
			$id=$r['id'].$r['val'];
    ?>
	<div class="form">
		    <form action="departmentinsert.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>DEPARTMENT_INSERT:</legend>
				DEPARTMENT_ID<br>
				<input class="input" type="text" name="d_id" value=<?php echo $id ?> readonly="readonly" required><br>
				DEPARTMENT_NAME<br>
				<input class="input" type="text" name="d_name" required><br>
				PHONE_NUMBER<br>
				<input class="input" type="number" name="ph_number" required><br>
				HEAD_OF_DEPARTMENT<br>
				<input class="input" type="text" name="hod" required><br>
