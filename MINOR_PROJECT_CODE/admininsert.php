...
<html>
	...
	$s3=$_POST['contact_no'];
	$c=$_SESSION["levels"];
	$sql="insert into admins values('$s1','$s2','$s3','$c')"; 
	if ($con5->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="admin id ".$s1." is inserted";
					$a=$_SESSION["user_id"];
		    ...
		    $qwery="update id_generator set val=val+1 where id='a'";
			$res=$con->query($qwery);
...
?>
	...
	<div class="article2">
	<?php
			$qwery="select * from id_generator where id='a'";
			$res=$con->query($qwery);
			$r=$res->fetch_assoc();
			$id=$r['id'].$r['val'];
    ?>
	<div class="form">
		    <form action="admininsert.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>ADMIN_INSERT:</legend>
				ADMIN_ID<br>
				<input class="input" type="text" name="a_id" value=<?php echo $id ?> readonly="readonly" required><br>
				ADMIN_NAME<br>
				<input class="input" type="text" name="a_name" required><br>
				CONTACT_NO<br>
				<input class="input" type="number" name="contact_no" required><br>
