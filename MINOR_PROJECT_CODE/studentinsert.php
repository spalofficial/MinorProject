...
<html>
	...
	$s7=$_POST['cgpa'];
	$c=$_SESSION["levels"];
	$sql="insert into student values('$s1','$s2','$s3','$s4','$s5','$s6','$s7','$c')"; 
	if ($con2->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="student id ".$s1." is inserted";
					$a=$_SESSION["user_id"];
		    ...
		    $qwery="update id_generator set val=val+1 where id='s'";
			$res=$con->query($qwery);
...
?>
	...
	<div class="article2">
	<?php
			$qwery="select * from id_generator where id='s'";
			$res=$con->query($qwery);
			$r=$res->fetch_assoc();
			$id=$r['id'].$r['val'];
    ?>
	<div class="form">
		   <form action="studentinsert.php" method="POST" style="top:60px;left:550px;">
		      <fieldset>
			   <legend>STUDENT_INSERT:</legend>
				REGN_ID<br>
				<input class="input" type="text" name="regn_id" value=<?php echo $id ?> readonly="readonly" required><br>
				NAME<br>
				<input class="input" type="text" name="name"  required><br>
				ADDRESS<br>
				<input class="input" type="text" name="address" required><br>
				YEAR_OF_PASSING<br>
				<input class="input" type="year" name="year_of_passing"  required><br>
				PHONE NUMBER<br>
				<input class="input" type="number" name="ph_number"  required><br>
				STATUS<br>
				<input class="input" type="text" name="status" required><br>
				CGPA<br>
				<input class="input" type="number" name="cgpa" required><br>
