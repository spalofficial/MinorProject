...
<html>
	...
	$c=$_SESSION["levels"];
	$sql="insert into course values('$s1','$s2','$s3','$c')"; 
	if ($con4->query($sql) === TRUE) {
		if($_SESSION['aset']){								
					$res="course id ".$s1." is inserted";
					$a=$_SESSION["user_id"];
		    ...
		    $qwery="update id_generator set val=val+1 where id='c'";
			$res=$con->query($qwery);
...
?>
	...
	<div class="article2">
	<?php
			$qwery="select * from id_generator where id='c'";
			$res=$con->query($qwery);
			$r=$res->fetch_assoc();
			$id=$r['id'].$r['val'];
    ?>
	<div class="form">
		    <form action="courseinsert.php" method="POST" style="top:60px;left:550px;">
			<fieldset>
			   <legend>COURSE_INSERT:</legend>
				COURSE_ID<br>
				<input class="input" type="text" name="c_id" value=<?php echo $id ?> readonly="readonly" required><br>
				COURSE_NAME<br>
				<input class="input" type="text" name="c_name" required><br>
				QUALIFICATION<br>
				<input class="input" type="text" name="qualification" required><br>
